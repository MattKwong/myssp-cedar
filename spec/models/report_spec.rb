...
describe Report do
  ...
  it "should return a list of all active programs" do
    report = Report.new
    test_prog1 = Program.active.first
  ...
  describe "spending calculation tests" do
    ...
    describe "purchases of non taxable items with no program specified" do
      before :each do
        @test_prog1 = Program.find_by_name("Test Site 1 Summer Domestic 2013")
        ...
        @purchase1 = Purchase.create!(:date => @start_date + 1, :program_id => @test_prog1.id, :purchaser_id => 1, :tax => 0, :total => 500.00,
    ...
    describe "purchases of taxable items with no program specified with multiple sites" do
      before :each do
        @test_prog1 = Program.find_by_name("Test Site 1 Summer Domestic 2013")
        @test_prog3 = Program.create!(@program_attr.merge(:site_id => Site.find_by_name('Test Site 3').id))
        @item2 = Item.create!(:name => "Test material item", :base_unit => 'each', :default_taxed => true, :description => 'Test material item description',
                              :budget_item_type_id => BudgetItemType.find_by_name("Materials").id, :untracked => true, :item_type_id => 1, :item_category_id => 1)
        @vendor1 = Vendor.create!(:site_id => @test_prog3.site_id, :name => 'Test Vendor 1', :address => '1 First St.', :city => 'Los Angeles', :state => 'CA',
                                  :zip => '90037', :phone => '123-456-7890' )
        @purchase2 = Purchase.create!(:date => @start_date + 1, :program_id => @test_prog3.id, :purchaser_id => 1, :tax => 8.50, :total => 100.00,
                                      :vendor_id => @vendor1.id, :purchase_type => 'Credit' )
      end
      ...
      it "given a budget item id, should return the amount spent without tax" do
        ...
        ItemPurchase.create!(:item_id => @item2.id, :price => 45.75, :purchase_id => @purchase2.id, :quantity => 2,
                             :taxable => true, :size => 'each', :uom => "each")
        @test_prog3.budget_item_spent(BudgetItemType.find_by_name("Materials").id, @start_date, @end_date).should == 91.50
      end
      ...
      it "given a budget item id, should return the amount spent with tax" do
        ...
        ItemPurchase.create!(:item_id => @item2.id, :price => 45.75, :purchase_id => @purchase2.id, :quantity => 2,
                             :taxable => true, :size => 'each', :uom => "each")
        @test_prog3.budget_item_spent_with_tax(BudgetItemType.find_by_name("Materials").id, @start_date, @end_date).should be_within(0.005).of(100.00)
      end
      ...
      it "should exclude purchase outside of date range" do
        ...
        ItemPurchase.create!(:item_id => @item2.id, :price => 45.75, :purchase_id => @purchase2.id, :quantity => 2,
                             :taxable => true, :size => 'each', :uom => "each")
        report.spending_with_tax_total(@start_date + 35, @end_date).should be_within(0.005).of(0.00)
      end
    end
  end
end
