        ...
        case
          ...
          when self =~ other
            ...
            if [self, other].any? {|x| x.is_temperature?}
              ...
              Unit.new(:scalar=>(self.base_scalar + other.base_scalar)*@q.to_d, :numerator=>@numerator, :denominator=>@denominator, :signature => @signature)
            end
          else
          ...
          when self =~ other
            case
              ...
              when other.is_temperature?
                ...
                puts "q value is #{@q}"
                #Unit.new(:scalar=>(self.base_scalar - other.base_scalar)*@q, :numerator=>@numerator, :denominator=>@denominator, :signature=>@signature)
                Unit.new(:scalar=>@q*(self.base_scalar - other.base_scalar), :numerator=>@numerator, :denominator=>@denominator, :signature=>@signature)
            end
          else
            raise ArgumentError,  "Incompatible Units ('#{self}' not compatible with '#{other}')"
        end
